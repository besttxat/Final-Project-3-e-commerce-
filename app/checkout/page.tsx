...
export default function CheckoutPage() {
    ...
    const handleCheckout = async (e: React.FormEvent) => {
        ...
        if (paymentMethod === 'card') {
            ...
            window.Omise.createToken('card', card, async (statusCode: number, response: any) => {
                if (statusCode === 200) {
            ...
            <div className="max-w-[500px] mx-auto border border-shop-border p-8 rounded-[20px]">
                <div className="flex gap-4 mb-8">
                    <button
                        ...
                        className={`flex-1 py-3 rounded-xl font-bold border ${paymentMethod === 'card' ? 'bg-black text-white border-black' : 'bg-white text-black border-shop-gray-200'}`}
                    >
                        Credit Card
                    </button>
                    <button
                        onClick={() => setPaymentMethod('promptpay')}
                        className={`flex-1 py-3 rounded-xl font-bold border ${paymentMethod === 'promptpay' ? 'bg-black text-white border-black' : 'bg-white text-black border-shop-gray-200'}`}
                    >
                        PromptPay
                    </button>
                </div>
